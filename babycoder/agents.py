...
def code_tasks_refactor_agent(objective: str, task_list_json) -> str:
    ...
    return openai_call(
        ...
        temperature=0,
        max_tokens=remaining_tokens(prompt),
    )
...
def task_assigner_recommendation_agent(objective: str, task: str) -> str:
    ...
    return openai_call(
        ...
        temperature=0.2,
        max_tokens=remaining_tokens(prompt, max_tokens=MAX_TOKENS_SIMPLE),
    )
...
def code_writer_agent(
    task: str, isolated_context: str, context_code_chunks: list | str
) -> str:
...
def code_relevance_agent(
    objective: str, task_description: str, code_chunk: dict
) -> str:
