from utils import openai_call, available_tokens
import os
import logging
...
def code_tasks_initializer_agent(objective: str) -> str:
    ...
    return openai_call(prompt, temperature=0.8, max_tokens=available_tokens(prompt))
...
def code_tasks_refactor_agent(objective: str, task_list_json) -> str:
    ...
    return openai_call(prompt, temperature=0, max_tokens=available_tokens(prompt))
...
def code_tasks_details_agent(objective: str, task_list_json) -> str:
    ...
    RETURN JSON ONLY:"""
    return openai_call(prompt, temperature=0.7, max_tokens=available_tokens(prompt))
...
def task_assigner_recommendation_agent(objective: str, task: str) -> str:
    ...
    return openai_call(prompt, model=OPENAI_API_MODEL_SIMPLE, temperature=0.5, max_tokens=3000)
...
def task_assigner_agent(objective: str, task: str, recommendation: str) -> str:
    ...
    return openai_call(prompt, model=OPENAI_API_MODEL_SIMPLE, temperature=0, max_tokens=2000)
...
def command_executor_agent(task: str, file_path: str) -> str:
    ...
    return openai_call(prompt, model=OPENAI_API_MODEL_SIMPLE, temperature=0, max_tokens=available_tokens(prompt, max_token=4000))


def code_writer_agent(task: str, isolated_context: str, context_code_chunks) -> str:
...
def code_refactor_agent(task_description: str, existing_code_snippet: dict, context_chunks, isolated_context: str) -> str:
    ...
    Existing code you should refactor (this is a snipplet from the file {existing_code_snippet["file_path"]}, line {existing_code_snippet["from_line"]} to line {existing_code_snippet["to_line"]}):
    ...
    Prepare the modifications to the code using the output from "diff -u" which can be applied as a patch to the original source code file.
    
    The source code is provided as an extract from original file including line numbers in original source code file, pretend you know the whole file when generating the required patch output.

    Respond only with JSON, use following format: {{"file_path": "{{file_to_be_modified}}", "patch":{{"output_from_diff"}}}}. IMPORTANT: JUST RETURN CODE AS PATCH, YOUR OUTPUT WILL BE ADDED DIRECTLY TO THE FILE BY OTHER AGENT. BE MINDFUL OF THIS:"""
...
def file_management_agent(objective: str, task: str, current_directory_files: str, file_path: str) -> str:
    ...
    return openai_call(prompt, model=OPENAI_API_MODEL_SIMPLE, temperature=0, max_tokens=available_tokens(prompt))
...
def task_human_input_agent(task: str, human_feedback: str) -> str:
    ...
    return openai_call(prompt, temperature=0.3, max_tokens=available_tokens(prompt))
