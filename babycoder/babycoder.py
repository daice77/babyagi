...
def code_tasks_initializer_agent(objective: str):
    ...
    return openai_call(prompt, temperature=0.8, max_tokens=3000)
...
def code_tasks_refactor_agent(objective: str, task_list_json):
    ...
    return openai_call(prompt, temperature=0, max_tokens=3000)
...
def code_tasks_details_agent(objective: str, task_list_json):
    ...
    RETURN JSON ONLY:"""
    return openai_call(prompt, temperature=0.7, max_tokens=3000)
...
def task_assigner_agent(objective: str, task: str, recommendation: str):
    ...
    return openai_call(prompt, model=OPENAI_API_MODEL_SIMPLE, temperature=0, max_tokens=3000)
...
def task_human_input_agent(task: str, human_feedback: str):
    ...
    return openai_call(prompt, temperature=0.3, max_tokens=1000)
...
for task in task_json["tasks"]:
    ...
    print_colored_text(f'*****TASK {task["id"]}/{len(task_json["tasks"])}*****', "yellow")
