...
if RESET_EMBEDDINGS_INDEX:
    ...
    try:
        playground_data_path = os.path.join(
            current_directory, 'playground_data')
    ...
    for task in task_json["tasks"]:
        ...
        if chosen_agent == "command_executor_agent":
            ...
            if chosen_agent == "code_writer_agent":
                ...
                current_directory_files = execute_command_string(
                    "find . -type f")
                ...
                print_colored_text(
                    "*****ANALYZING EXISTING CODE*****", "yellow")
                relevance_scores = []
                ...
                relevance_scores_dict = [
                    json.loads(r) for r in relevance_scores]
                relevance_scores_int = [{'relevance_score': int(
                    x['relevance_score'])} for x in relevance_scores_dict]
                results = [{**score, **chunk} for score,
                           chunk in zip(relevance_scores_int, code_chunks)]
